...
int buscaListaR(LinkedNode *inicio, int ra, int cont){
	if (inicio == NULL){
		return cont;
	}
	if (inicio->ra == )
}/*/
int buscaLista2(LinkedNode *inicio, int ra){
	int c = 1;
	if (inicio->ra > ra){
		return 1;
	}
	LinkedNode *curr = inicio;
	while(curr->next != 0 && curr->ra < ra){
		c++;
		curr = curr->next;
	}
	return c;
...
int main(){
	...
	while(operador != 'P' && ra != 0){
		if (operador == 'I'){
			...
			//printf("B %d\n", ra);
			printf("L=%d L2=%d A=%d\n", buscaLista(inicio, ra),buscaLista2(inicio, ra),buscarTreeNode(raiz, ra, 0));
		}
		if (operador == 'Z')
		{
			imprimeLista(inicio);
